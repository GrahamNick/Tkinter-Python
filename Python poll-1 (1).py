...
##    while True:
##        
##        if name != 'quit' and name != '':
##            print('Please type a name')
##        else:
##            break
##    if name == 'quit':
##        break   
##    if name.lower() in HoSPN:
##        print('That name already exists. Please use a different name.')
##        name = input('What is your name? ')
##        continue
##    while True:
##        hours = input('How many hours of sleep per night do you usually get when you have school the next day? ')
##        try:
##            hours = float(hours)
##            break
##        except:
...
##    
##    HoSPN[name.lower()] = hours
##    f = open('Data.txt','w')
##    f.write(str(HoSPN))
##    f.close()
##for i,j in HoSPN.items():
##    average += HoSPN[i]
##if len(HoSPN) > 0:
##    if len(HoSPN) < 2:
##        person = 'person'
##    else:
##        person = 'people'
##    print('Out of ' + str(len(HoSPN)) + ' ' + person + '. The average amount of hours slept is: ' + str(average/len(HoSPN)))
